...
public class StatisticRebuilder {
    ...
    private final ConcurrentHashMap<Long, RebuildInfo> activeRebuilds = new ConcurrentHashMap<>();
    private final ReentrantLock infoCreateLock = new ReentrantLock();
    ...
    private void parseReaction(@NotNull ReactionAddEvent event) {
        event.getServer().ifPresent(server ->
                event.getServerTextChannel().ifPresent(channel -> {
                    ...
                    Optional<String> mayBeEmoji = event.getEmoji().asUnicodeEmoji();
                    OptionalUtils.ifPresentOrElse(mayBeEmoji, em -> {
    ...
    private boolean processingMessage(@NotNull Message hist, @NotNull ServerTextChannel channel, @NotNull Server server) {
        ...
        hist.getReactions().forEach((reaction) ->
